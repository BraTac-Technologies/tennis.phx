defmodule TennisPhx.MatchesTest do
  ...
  describe "groups" do
    ...
    test "list_groups/0 returns all groups" do
      group = group_fixture()
      assert Matches.list_groups() == [group]
    end

    test "get_group!/1 returns the group with given id" do
      group = group_fixture()
      assert Matches.get_group!(group.id) == group
    end

    test "create_group/1 with valid data creates a group" do
      valid_attrs = %{name: "some name"}

      assert {:ok, %Group{} = group} = Matches.create_group(valid_attrs)
      assert group.name == "some name"
    end

    test "create_group/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Matches.create_group(@invalid_attrs)
    end
    ...
    test "update_group/2 with valid data updates the group" do
      ...
      assert {:ok, %Group{} = group} = Matches.update_group(group, update_attrs)
      assert group.name == "some updated name"
    end

    test "update_group/2 with invalid data returns error changeset" do
      group = group_fixture()
      assert {:error, %Ecto.Changeset{}} = Matches.update_group(group, @invalid_attrs)
      assert group == Matches.get_group!(group.id)
    end

    test "delete_group/1 deletes the group" do
      group = group_fixture()
      assert {:ok, %Group{}} = Matches.delete_group(group)
